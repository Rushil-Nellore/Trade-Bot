...
def save_trade_log(result: SimulationResult, path: str = TRADE_LOG_PATH) -> None:
    if not result.trade_log:
        logger.warning("No completed trades — writing an empty trade log to %s.", path)
...
def plot(df: pd.DataFrame, result: SimulationResult, path: str = DASHBOARD_PATH) -> None:
    ...
    if result.balance_history:
        ...
        ax2.fill_between(
            ...
            INITIAL_BALANCE,
            bal_values,
            where=[v >= INITIAL_BALANCE for v in bal_values],
            color="#00ff88",
            alpha=0.15,
        )
        ax2.fill_between(
    ...
    ax2.set_title(
        f"Portfolio Balance  |  Sharpe {result.sharpe_ratio:.2f}"
